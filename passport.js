...
const SHEET_BASE_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vROu-VwOiztKC_a-ncHZqcjWniIVDVxVdmaN9sJ__n_mbM-IZF9-jnnomyl-zWb5oxk0rkVBlh5ovKP/pub?output=csv";
const SHEET_CSV_URL = SHEET_BASE_URL + "&gid=866228631";
// These two sheets are read by name. If your Google Sheet is published as a
// whole document, Google serves them through the same published endpoint.
const MISSION_PROGRESS_CSV_URL = SHEET_BASE_URL + "&sheet=MISSION%20PROGRESS";
const FORM_RESPONSES_CSV_URL = SHEET_BASE_URL + "&sheet=Form%20Responses%201";
...
function renderDiscoveries(student, missionRows) {
  ...
  const relevant = missionRows.filter(r => {
    const rowName = findHeader(r, "Nama");
    const rowNpm = findHeader(r, "NPM");
    return (npm && rowNpm && rowNpm === npm) || (name && rowName && cleanId(rowName) === cleanId(name));
  });
  const completed = relevant.filter(r => String(findHeader(r, "Status")).trim().toLowerCase() === "selesai" && findHeader(r, "Discovery / Hasil"));
  if (!completed.length) {
  ...
  el.innerHTML = completed.map(r => {
    const mission = findHeader(r, "Mission") || "Digital Mission";
    const result = findHeader(r, "Discovery / Hasil");
    return `<article class="discovery"><span>${escapeHTML(mission.toUpperCase())}</span><p>“${escapeHTML(result)}”</p></article>`;
  }).join("");
}
...
function renderBusinessIdea(student, formRows) {
  ...
  const name = findHeader(student, "Nama");
  const npm = findHeader(student, "NPM");
  const row = formRows.find(r => {
    const rowName = findHeader(r, "Nama Lengkap");
    const rowNpm = findHeader(r, "NPM");
    return (npm && rowNpm && rowNpm === npm) || (name && rowName && cleanId(rowName) === cleanId(name));
  });
  const idea = row ? findHeader(row, "Jika saya bisa membuat bisnis digital...") : "";
  if (!idea) {
